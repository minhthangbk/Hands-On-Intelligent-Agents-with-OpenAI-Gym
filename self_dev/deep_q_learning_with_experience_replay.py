...
class ReplayBuffer:
    ...
    def sample_batch(self) -> Dict[str, np.ndarray]:
        idxs = np.random.choice(self.size, size=self.batch_size, replace=False)  # Select index arbitrarily
...
class DQNAgent:
    ...
    def _compute_dqn_loss(self, samples: Dict[str, np.ndarray]) -> torch.Tensor:  # return torch Tensor data type
        ...
        next_q_value_target = self.dqn_target(next_state).max(dim=1, keepdim=True)[0].detach()  # select the maximum q values
